...
def test_contributions_over_time_with_dim(mock_suite: LegacyMMMPlotSuite):
...
def test_contributions_over_time_with_dims_list(mock_suite: LegacyMMMPlotSuite):
...
def test_contributions_over_time_with_multiple_dims_lists(
    mock_suite: LegacyMMMPlotSuite,
):
...
def mock_suite(mock_idata):
    """Fixture to create a mock LegacyMMMPlotSuite with a mocked posterior."""
    return LegacyMMMPlotSuite(idata=mock_idata)


@pytest.fixture(scope="module")
def mock_suite_with_sensitivity(mock_idata_with_sensitivity):
    """Fixture to create a mock LegacyMMMPlotSuite with sensitivity analysis."""
    return LegacyMMMPlotSuite(idata=mock_idata_with_sensitivity)


def test_contributions_over_time_expand_dims(mock_suite: LegacyMMMPlotSuite):
    fig, ax = mock_suite.contributions_over_time(
        var=[
            "intercept",
            "linear_trend",
...
def mock_suite_with_constant_data(mock_idata_with_constant_data):
    """Fixture to create a LegacyMMMPlotSuite with constant_data for saturation tests."""
    return LegacyMMMPlotSuite(idata=mock_idata_with_constant_data)
...
class TestSaturationScatterplot:
    ...
    def test_saturation_scatterplot_no_original_scale_contribution(
        ...
        suite_without_original_scale = LegacyMMMPlotSuite(idata=idata_copy)
...
def mock_suite_with_constant_data_single_dim(mock_idata_with_constant_data_single_dim):
    return LegacyMMMPlotSuite(idata=mock_idata_with_constant_data_single_dim)
...
def test_sensitivity_analysis_error_on_missing_results(mock_idata):
    suite = LegacyMMMPlotSuite(idata=mock_idata)
...
def test__validate_dims_valid():
    ...
    suite = LegacyMMMPlotSuite(idata=None)
