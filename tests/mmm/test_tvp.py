...
def test_time_varying_prior(coords):
    with pm.Model(coords=coords) as model:
        X = XData("X", np.array([0, 1, 2, 3, 4]), dims="date")
        ...
        pm.sample(50, tune=50, chains=1, quiet=True)
...
def test_multidimensional(coords):
    with pm.Model(coords=coords) as model:
        X = XData("X", np.array([0, 1, 2, 3, 4.0]), dims="date")
        m = 7
        ...
        # Assert internal parameters are created correctly
        assert model.test_raw_hsgp_coefs.eval().shape == (m, 3)
...
def test_create_time_varying_intercept(coords, model_config):
    ...
    with pm.Model(coords=coords):
        time_index = XData("X", np.array([0, 1, 2, 3, 4]), dims="date")
