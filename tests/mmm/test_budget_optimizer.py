...
def test_dict_bounds_respect_channel_ordering(mock_pymc_sample):
    """Regression test for #2323: dict bounds must apply to correct channels
    regardless of alphabetical ordering."""
    rng = np.random.default_rng(42)
    num_obs = 12
    # Non-alphabetical channel names to expose ordering bug
    channels = ["Z_channel", "A_channel"]
    X = rng.uniform(0.0, 1.0, size=(num_obs, len(channels)))
    true_beta = np.array([0.8, 0.2])
    y = (X @ true_beta) + rng.normal(0.0, 0.05, size=num_obs)
    ...
    # Asymmetric bounds: Z_channel up to 80, A_channel only up to 20
    bounds = {"Z_channel": (0.0, 80.0), "A_channel": (0.0, 20.0)}

    optimal_budgets, _result = optimizer.allocate_budget(
        total_budget=100.0, budget_bounds=bounds
    )

    # Each channel must respect its own bounds
    z_val = optimal_budgets.sel(channel="Z_channel").item()
    a_val = optimal_budgets.sel(channel="A_channel").item()
    assert z_val <= 80.0 + 1e-6, f"Z_channel={z_val} exceeds bound 80"
    assert a_val <= 20.0 + 1e-6, f"A_channel={a_val} exceeds bound 20"
