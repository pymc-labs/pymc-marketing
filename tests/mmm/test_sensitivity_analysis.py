...
def mock_mmm():
    ...
    class MockMMM:
        def __init__(self):
            ...
            class MockIdata:
                def __init__(self):
                    self.posterior_predictive = xr.Dataset(
                        {"y": xr.DataArray([10, 20, 30], dims=["dim_0"])}
                    )
        ...
        def predict(self, X_new, extend_idata=False, progressbar=False):
            # Mock predict method to return a simple transformation of X_new
            return xr.DataArray(
                X_new["predictor_1"] + X_new["predictor_2"],
                dims=["dim_0"],
            )
