...
def test_n_samples_clamped_to_available(self, simple_plots, simple_curve):
    n_available = 2 * 50  # chains * draws in simple_curve
    _, axes = simple_plots.saturation_curves(
        curves=simple_curve, n_samples=10_000, hdi_prob=None
    )
    for ax in axes.flat:
        lines = ax.get_lines()
        # mean line + clamped samples
...
def test_mean_curve_y_matches_curves_mean(self, simple_plots, simple_curve):
    from pymc_marketing.mmm.plotting.transformations import _ensure_chain_draw_dims
    curves_cd = _ensure_chain_draw_dims(simple_curve)
    expected_mean = curves_cd.mean(dim=["chain", "draw"])
...
For each panel, extract the y-bounds of the HDI polygon and verify the mean curve falls within them everywhere.
...
def test_hdi_band_bounds_contain_mean(self, simple_plots, simple_curve):
    ...
    for ax, ch in zip(axes.flat, channels):
        polys = [c for c in ax.collections if "Poly" in type(c).__name__]
        assert len(polys) > 0
        verts = polys[0].get_paths()[0].vertices  # shape (2*n+1, 2)
        n = len(expected_mean.sel(channel=ch))
        y_lower = verts[:n, 1]
        y_upper = verts[n : 2 * n, 1][::-1]
...
### 3.2 `TestSaturationScatterplotCostPerUnit` — add label assertions
...
def test_apply_cost_per_unit_false(self, panel_plots):
    _, axes = panel_plots.saturation_scatterplot(apply_cost_per_unit=False)
    for ax in axes.flat:
...
**Current:** `assert isinstance(fig, Figure)`
**Upgraded:** wrap with `warnings.catch_warnings(record=True)` and assert no `UserWarning` — confirming the no-warning path works for sample-dim curves in scaled space.
...
| New tests added to existing classes | 5 (invalid dim key for scatter, figsize conflict warning, curves idata mutation, cost-per-unit labels × 2) |
| Weak tests upgraded in-place | 4 |
