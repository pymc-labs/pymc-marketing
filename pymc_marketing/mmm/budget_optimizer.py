...
def cost_function(budget_allocations: List[float], df: DataFrame, data: DataFrame, channel_contributions: DataArray) -> float:
    """
    ...
    Notes
    -----
    This function calculates the total collaboration of budget allocations for each channel based on a quadratic curve fitted to the channel data. The total collaboration is computed as the sum of the polynomial evaluation at the allocation for each channel.
    """
...
def budget_constraint(budget_allocations: List[float], total_budget: float) -> float:
    ...
    Notes
    -----
    This function calculates the difference between the total allocated budget and the total available budget. This difference should be zero in the optimization problem.
    """
