...
class LinearTrend(BaseModel):
    ...
    priors: InstanceOf[dict[str, XPrior | Prior]] = Field(
        None,
    ...
    def _priors_are_set(self) -> Self:
        priors = self.priors or self.default_priors
        xpriors = {}
        for key, prior in priors.items():
            if isinstance(prior, Prior):
                warnings.warn(
                    "Prior {prior} automatically converted to XPrior. Use XPrior to avoid warning.",
                    UserWarning,
                    stacklevel=2,
                )
    ...
    @property
    def default_priors(self) -> dict[str, XPrior]:
        ...
        if self.include_intercept:
            priors["k"] = XPrior("Normal", mu=0, sigma=0.05)
