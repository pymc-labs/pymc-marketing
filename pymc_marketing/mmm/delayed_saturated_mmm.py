...
class DelayedSaturatedMMM(
    ...
    def build_model(
        ...
        if "coords" in model_config.keys():
        ...
        with pm.Model(coords=coords) as self.model:
            channel_data_ = pm.MutableData(
                name="channel_data",
                value=model_data["channel_data"]["value"],
                dims=model_data["channel_data"]["dims"],
            )

            target_ = pm.MutableData(
                name="target",
                value=model_data["target"]["value"],
                dims=model_data["target"]["dims"],
            )
            ...
            channel_adstock = pm.Deterministic(
                ...
                var=geometric_adstock(
                    x=channel_data_,
                    alpha=alpha,
                    l_max=model_config["channel_adstock"]["var"]["l_max"],
                    normalize=True,
                    axis=0,
                ),
                dims=("date", "channel"),
            ...
            if "control_data" in model_data.keys():
    ...
    def create_sample_input(
        self, data=None, adstock_max_lag: int = 4
