...
class SensitivityAnalysis:
    ...
    def _draw_indices_for_percentage(
        self, posterior_sample_percentage: float
    ) -> slice | np.ndarray:
        if not (0 < posterior_sample_percentage <= 1):
            raise ValueError(
                "'posterior_sample_percentage' must be in the interval (0, 1]."
            )
        ...
        retained_fraction = posterior_sample_percentage**2
    ...
    def _extract_response_distribution(
        self, response_variable: str, posterior_sample_percentage: float
    ):
        """Extract response distribution graph conditioned on posterior samples."""
        draw_selection = self._draw_indices_for_percentage(posterior_sample_percentage)
    ...
    def run_sweep(
        ...
        *,
        ...
        posterior_sample_percentage: float = 1.0,
        ...
        **kwargs,
        ...
        posterior_sample_percentage : float
            Posterior sampling control in (0, 1]. The retained fraction of draws is
            roughly ``posterior_sample_percentage ** 2`` (e.g., 0.1 keeps about 1% of
        ...
        if "posterior_sample_batch" in kwargs:
            ...
            warnings.warn(
                "'posterior_sample_batch' is deprecated; use 'posterior_sample_percentage' instead.",
                DeprecationWarning,
                stacklevel=2,
            )
            ...
            posterior_sample_percentage = min(1.0, 1.0 / math.sqrt(legacy_value))
        ...
        resp_graph = self._extract_response_distribution(
            ...
            posterior_sample_percentage=posterior_sample_percentage,
        )
