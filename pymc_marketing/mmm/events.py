...
class GaussianBasis(Basis):
    ...
    def function(
        ...
        out = pmd.math.exp(logp(rv, x.broadcast_like(rv)))
        return out
...
class HalfGaussianBasis(Basis):
    ...
    def function(
        ...
        x = as_xtensor(x)
        rv = pmd.Normal.dist(mu=0.0, sigma=sigma, dim_lengths=x.sizes)
        out = ptx.math.exp(logp(rv, x.broadcast_like(rv)))
    ...
    def function(
        ...
        *,
        ...
        y_before = ptx.math.switch(
            indicator_before,
            ptx.math.exp(logp(rv_before, x.broadcast_like(rv_before))),
            0,
        )
        y_after = ptx.math.switch(
            indicator_after,
            ptx.math.exp(logp(rv_after, x.broadcast_like(rv_after))) * a_after,
            0,
        )
