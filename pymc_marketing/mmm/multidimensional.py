...
from pymc_marketing.mmm.dims import XData, XPrior, XTensorLike
    ...
    def build_model(  # type: ignore[override]
        ...
        **kwargs : dict
        ...
        with pm.Model(
            ...
            _channel_scale = XData("channel_scale", self.scalers._channel)
            _target_scale = XData("target_scale", self.scalers._target)

            _channel_data = XData("channel_data", self.xarray_dataset._channel)

            _target = XData("target_data", self.xarray_dataset._target)
            ...
            if self.time_varying_intercept or self.time_varying_media:
                time_index = XData("time_index", self._time_index)
            ...
            if self.control_columns is not None and len(self.control_columns) > 0:
                ...
                control_data_ = XData("control_data", self.xarray_dataset._control)
            ...
            if self.yearly_seasonality is not None:
                ...
                fourier_contributions = pmd.Deterministic(
                    ...
                    self.yearly_fourier.apply(dayofyear.values, sum=False).transpose(
                        "date", ..., fourier_dim
                    ),
                )
