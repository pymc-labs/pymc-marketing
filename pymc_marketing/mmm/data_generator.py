...
class DataGenerator:
    ...
    def add_channel_spends(self, num_channels=2, beta_scale=3):
        ...
        self.channel_spend_probs = self.rng.uniform(.5, 1.0, num_channels)
    ...
    def plot_channel_spends(
        ...
        if ax is None:
            fig, ax = plt.subplots(
                nrows=self.num_channels, ncols=1, figsize=FIGSIZE
    ...
    def add_lag_effect(self, fn='geometric', max_lag:int=8):
        ...
        ''';
        assert fn in self.lag_fns.keys(), f'Lag function must be in \
        ("geometric", "adstock"). Function unrecognized: {fn}'
        df = self.df
        self.is_lagged = True # signals that adstock has been applied
        self.lag_fn = fn
        self.alphas = self.rng.beta(.5, .5, self.num_channels)
        self.thetas = None
        if fn == 'adstock':
            # trying to keep delays between 0 and 3 for the most part
            self.thetas = self.rng.binomial(max_lag-1, .1, self.num_channels)
        ...
        for c in range(self.num_channels):
            ...
            if fn == 'geometric':
                ...
                fn = delayed_adstock
                assert self.thetas is not None, 'thetas must be specified to apply adstock.'
    ...
    def plot_lag_effect(self):
        ...
        self.plot_channel_spends(
                suffix="_lag", fig=fig
                , ax=ax, title=f'{self.lag_fn.capitalize()} Effect', alpha=.5)
    ...
    def plot_saturation_effect(self):
        ...
        suffix=''
        if self.is_lagged: suffix='_lag'
        fig, ax = self.plot_channel_spends(suffix=suffix, show=False)
        suffix+="_saturated"
        self.plot_channel_spends(suffix=suffix, alpha=.5, title="Saturation Effect", fig=fig, ax=ax)
    ...
    def plot_trend_and_seasonality_effects(self, show=True):
        ...
        plt.plot(df["date_week"], df["trend"], color="C3", label="trend", alpha=.4)
        plt.plot(df["date_week"], df["seasonality"], color="C4", label="seasonality",
                     alpha=.5)
        plt.plot(df["date_week"], df.seasonality+df.trend, color="C5",
    ...
    def plot_baseline(self, show:bool=True):
        ...
        plt.fill_between(x=df.date_week, y1=fillmin, y2=df.baseline,
                         color='C1', label='baseline', alpha=.7)
        plt.legend()
        if show: plt.show()
    ...
    def plot_channel_decomposition(self, show:bool=True):
        ...
        for i in range(self.num_channels):
            (betas[i]*df[f'x{i}_lag_saturated']).plot(alpha=.5, label=f'x{i}')
    ...
    def calc_campaign_effectiveness(self):
        ...
        total_channel_mass = np.einsum('tc, c -> ', np.array(df[channel_names].values), self.betas)
    ...
    def get_true_params(self):
        ...
        if self.is_lagged:
            ...
            if self.lag_fn == 'adstock': d['thetas'] = self.thetas
    ...
    def get_param_descriptions(self):
        ...
        if self.is_trended_and_seasonal:
            ...
            d['seasonality_coefs'] = 'Coefficients of the Fourier seasonal components. [(sin1, cos1), (sin2, cos2)...]'
        if self.has_events:
    ...
    def generate_typical_mmm_dataset(
        ...
        if is_lagged: dg.add_lag_effect(fn=lag_fn)
